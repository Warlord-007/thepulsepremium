...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      /* 1. Callboard */ 
      try {
        const { CallboardClient } = await import('@wolpertingerlabs/callboard')
        const cb = new CallboardClient({ agentId: activeAgent })
        text = await cb.chat(content, { history })
        provider = 'Callboard'
      } catch {
        ...
        /* 3. Pollinations.ai FREE — always available, no key */
        if (!text) {
      ...
      setMessages(prev => ({
        ...prev,
        [activeAgent]: [...prev[activeAgent], { id: (Date.now()+1).toString(), role: 'assistant', content: text, timestamp: new Date(), agentId: activeAgent, provider }],
      }))
    } catch (err) {
      setMessages(prev => ({
        ...prev,
        [activeAgent]: [...prev[activeAgent], { id: (Date.now()+1).toString(), role: 'assistant', content: "All AI providers are briefly unavailable. Add VITE_GEMINI_API_KEY to your .env for reliable free access, or try again in a moment.", timestamp: new Date(), agentId: activeAgent, provider: 'error' }],
      }))
    } finally {
      setIsLoading(false)
    }
